...
def test_cost_usd_summing(tmp_path: Path) -> None:
    entries = [
        {"timestamp": "2025-01-01T00:00:00Z", "costUSD": 0.05,
         "message": {"usage": {"input_tokens": 100, "output_tokens": 50}, "model": "claude-sonnet-4-6"}},
        {"timestamp": "2025-01-01T00:01:00Z", "costUSD": 0.10,
         "message": {"usage": {"input_tokens": 200, "output_tokens": 80}, "model": "claude-sonnet-4-6"}},
    ]
...
def test_sidechain_tracked_separately(tmp_path: Path) -> None:
    ...
    entries = [
        {"timestamp": "2025-01-01T00:00:00Z", "costUSD": 0.05,
         "message": {"usage": {"input_tokens": 100, "output_tokens": 50}, "model": "claude-opus-4-6"}},
        {"timestamp": "2025-01-01T00:01:00Z", "isSidechain": True, "costUSD": 0.02,
         "message": {"usage": {"input_tokens": 500, "output_tokens": 200}, "model": "claude-haiku-4-5-20251001"}},
        {"timestamp": "2025-01-01T00:02:00Z", "isApiErrorMessage": True, "costUSD": 888.0,
         "message": {"usage": {"input_tokens": 8888, "output_tokens": 8888}, "model": "claude-sonnet-4-6"}},
    ]
    f = _write_jsonl(tmp_path / "session-abc123.jsonl", entries)
    snap = parse_claude_transcript(f)
    assert snap is not None
...
def test_malformed_lines(tmp_path: Path) -> None:
    ...
    content += json.dumps({
        "timestamp": "2025-01-01T00:00:00Z",
        "message": {"usage": {"input_tokens": 50, "output_tokens": 25}, "model": "claude-opus-4-6"},
    })
    ...
    entries = [
        ...
        {"timestamp": "2025-01-01T00:01:00Z",
         "message": {"usage": {
             ...
             "cache_read_input_tokens": 18000, "cache_creation_input_tokens": 2000,
         }, "model": "claude-opus-4-6"}},
    ]
