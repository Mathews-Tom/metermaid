...
Use `uv run python scripts/audit_source_schema.py <source-jsonl>` only to inspect a user-selected local source. The command prints a field name verbatim only when it is an ASCII Python identifier of at most 64 characters; every other field name is replaced with `<redacted-field-N>`. It never prints source values or the source path, and it does not write a fixture, archive, or copy of the source. Reviewers must still confirm that emitted identifier-shaped field names reveal no private information.
