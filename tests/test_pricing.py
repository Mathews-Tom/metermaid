...
def _snap(provider: str = "codex", model: str = "gpt-5.4",
          tokens_in: int = 1_000_000, tokens_out: int = 100_000,
          cache_read: int = 0, cost_usd: float = 0.0) -> Snapshot:
    return Snapshot(
        timestamp="2025-01-01T00:00:00", provider=provider, model=model,
        session_id="test", cost_usd=cost_usd, ctx_pct=0.0, ctx_tokens=0,
        ctx_max=0, wall_sec=0.0, api_sec=0.0, tokens_in=tokens_in,
        tokens_out=tokens_out, cache_read=cache_read, cache_write=0,
...
def test_known_claude_model() -> None:
    cost = estimate_cost("claude-opus-4-6", "claude", tokens_in=1_000_000, tokens_out=100_000)
...
def test_cache_read_discounted_for_claude() -> None:
    ...
    cost = estimate_cost("claude-sonnet-4-6", "claude",
                          tokens_in=1_000_000, tokens_out=0, cache_read=900_000)
...
def test_cache_read_50pct_discount_for_codex() -> None:
    ...
    cost = estimate_cost("gpt-5.4", "codex",
                          tokens_in=1_000_000, tokens_out=0, cache_read=900_000)
...
def test_fill_cost_preserves_real_cost() -> None:
    row = {"cost_usd": "1.234", "model": "claude-opus-4-6", "provider": "claude",
           "tokens_in": "999999", "tokens_out": "999999", "cache_read": "0"}
    assert fill_cost(row) == 1.234


def test_fill_cost_estimates_when_zero() -> None:
    row = {"cost_usd": "0", "model": "gpt-5.4", "provider": "codex",
           "tokens_in": "1000000", "tokens_out": "100000", "cache_read": "0"}
