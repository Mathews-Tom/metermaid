...
def _make_claude_jsonl(path: Path, tokens_in: int = 100, tokens_out: int = 50) -> Path:
    entries = [
        {"timestamp": "2025-01-01T00:00:00Z",
         "message": {"usage": {"input_tokens": tokens_in, "output_tokens": tokens_out},
...
def test_snap_hash_deterministic() -> None:
    snap = Snapshot(
        ...
        provider="claude", model="test", session_id="abc",
        cost_usd=0.0, ctx_pct=0.0, ctx_tokens=1000, ctx_max=200000,
        wall_sec=0.0, api_sec=0.0, tokens_in=100, tokens_out=50,
        cache_read=0, cache_write=0, diff_add=0, diff_del=0,
        path="", source="watcher",
...
def test_per_session_file_isolation(tmp_path: Path) -> None:
    ...
    snap_a = Snapshot(
        ...
        session_id="aaa", cost_usd=0.1, ctx_pct=0.0, ctx_tokens=0,
        ctx_max=0, wall_sec=0.0, api_sec=0.0, tokens_in=100,
        tokens_out=50, cache_read=0, cache_write=0, diff_add=0,
        diff_del=0, path="", source="watcher",
        tok_in_delta=0, tok_out_delta=0,
        sc_tokens_in=0, sc_tokens_out=0, sc_cost_usd=0.0, sc_models="",
    )
    snap_b = Snapshot(
        timestamp="2025-01-01T00:01:00", provider="codex", model="test",
        session_id="bbb", cost_usd=0.0, ctx_pct=0.0, ctx_tokens=0,
        ctx_max=0, wall_sec=0.0, api_sec=0.0, tokens_in=200,
        tokens_out=80, cache_read=0, cache_write=0, diff_add=0,
        diff_del=0, path="", source="watcher",
        tok_in_delta=0, tok_out_delta=0,
        sc_tokens_in=0, sc_tokens_out=0, sc_cost_usd=0.0, sc_models="",
    )
...
def test_multiple_rows_per_session(tmp_path: Path) -> None:
    ...
    for i in range(3):
        snap = Snapshot(
            timestamp=f"2025-01-01T00:0{i}:00", provider="claude", model="test",
            session_id="xxx", cost_usd=0.0, ctx_pct=0.0, ctx_tokens=0,
            ctx_max=0, wall_sec=0.0, api_sec=0.0, tokens_in=100 * (i + 1),
            tokens_out=50, cache_read=0, cache_write=0, diff_add=0,
            diff_del=0, path="", source="watcher",
            tok_in_delta=0, tok_out_delta=0,
        sc_tokens_in=0, sc_tokens_out=0, sc_cost_usd=0.0, sc_models="",
