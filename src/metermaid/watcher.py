...
def _live_table(
    snapshots: dict[str, Snapshot], last_change: dict[str, datetime], polls: int
) -> Table:
    now = datetime.now()
    ...
    total = len(ranked)
    t = Table(
        title=f"metermaid watcher — {n_active} active, {total} tracked (poll #{polls})"
    )
    ...
    for s, changed, mins in ranked[:_MAX_ROWS]:
        ...
        if mins < _STALE_MINUTES:
            ...
            t.add_row(
                ...
                f"[dim]{s.provider}[/dim]",
                f"[dim]{s.model[:22]}[/dim]",
                f"[dim]{s.tokens_in:,}[/dim]",
                f"[dim]{s.tokens_out:,}[/dim]",
                f"[dim]{cost}[/dim]",
            )
    if total > _MAX_ROWS:
...
class SessionWatcher:
    ...
    def _loop_live(self) -> None:
        ...
        with Live(
            _live_table(self._snaps, lc, 0), console=console, refresh_per_second=1
        ) as live:
            while self._running:
                n += 1
                self._poll()
    ...
    def _poll(self) -> int:
        ...
        recorded = 0
        for files, parser in [
            (claude_files, parse_claude_transcript),
    ...
    def _check(
        self, path: Path, parser: Callable[[Path], Snapshot | None]
    ) -> Snapshot | None:
        key = str(path)
        try:
    ...
    def _daemon_unix(self, log_path: Path) -> None:
        pid = os.fork()
        if pid > 0:
            console.print(f"Watcher started (PID {pid})")
            sys.exit(0)
        os.setsid()
    ...
    def _daemon_win(self, log_path: Path) -> None:
        ...
        with open(log_path, "a") as lf:
            proc = subprocess.Popen(
                cmd, stdout=lf, stderr=lf, stdin=subprocess.DEVNULL, creationflags=flags
            )
